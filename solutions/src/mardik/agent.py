...
class Agent:
    ...
    def _invoke_llm_sync(self, messages: list[dict[str, Any]]) -> Reply:
        with self.telemetry.tracer.start_as_current_span("llm.invoke"):
            try:
                ...
                self.telemetry.errors.add(1, attributes={"kind": "timeout"})
                raise LLMTimeoutError("LLM call timed out") from exc

    def _invoke_llm(self, messages: list[dict[str, Any]]) -> Reply:
        # The Azure SDK call is blocking, so run it on a worker thread — but
        # carry the current OpenTelemetry context across so the llm.invoke span
        # stays attached to the agent.turn span instead of starting a new trace.
    ...
    def _dispatch_tool(self, call: dict[str, Any]) -> str:
        with self.telemetry.tracer.start_as_current_span(
            "tool.call", attributes={"tool.name": call["name"]}
        ):
